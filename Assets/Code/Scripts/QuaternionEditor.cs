...
{
    [Range(-360, 360)] public float angle = 0f;
    ...
    private void OnEnable()
    {
        ...
        propAngle = obj.FindProperty("angle");
        propAxis = obj.FindProperty("axis");
    ...
    public void SceneGUI(SceneView view)
    {
        ...
        {
            vertices[i] = HQuaternion.Rotate(vertices[i], axis, radian);
...
public struct HQuaternion
{
    ...
    public HQuaternion(Vector3 vector, float w)
    {
        this.x = vector.x;
        this.y = vector.y;
        this.z = vector.z;
    ...
    public static Vector3 Rotate(Vector3 point, Vector3 axis, float angle)
    {
        ...
        HQuaternion p = new HQuaternion(point, 0f);

        HQuaternion rotatePoint = Multiplication(q, p);
        rotatePoint = Multiplication(rotatePoint, _q);

        return new(rotatePoint.x, rotatePoint.y, rotatePoint.z);
    }
}